...
// Dispatched whenever the layout changes.
    ...
    public class PodLayoutManager extends EventDispatcher
    {
        ...
        private function removeAllEventListener(pod:Pod):void
        {
            pod.removeEventListener(DragEvent.DRAG_START, onDragStartPod);
            pod.removeEventListener(DragEvent.DRAG_COMPLETE, onDragCompletePod);
            pod.removeEventListener(PodStateChangeEvent.MAXIMIZE, onMaximizePod);
            pod.removeEventListener(PodStateChangeEvent.MINIMIZE, onMinimizePod);
            pod.removeEventListener(PodStateChangeEvent.RESTORE, onRestorePod);
            pod.removeEventListener(PodStateChangeEvent.CLOSE, onClosePod);
        }
        ...
        private function onClosePod(e:PodStateChangeEvent):void
        {
            ...
            if (pod.windowState == Pod.WINDOW_STATE_MAXIMIZED) // Current state is maximized
            {
                ...
                pod.removeAllChildren();
                this.container.removeChild(pod);
                removeAllEventListener(pod);
                this.container.removeChild(dragHighlightItems.pop());
                this.updateLayout(true);
                dispatchEvent(new LayoutChangeEvent(LayoutChangeEvent.UPDATE));
            }
            else if (pod.windowState == Pod.WINDOW_STATE_DEFAULT)
            {
            ...
            if (currentDropIndex != dropIndex) // Make sure we have a new drop index so we don't create redudant effects.
            {
                ...
                for (i = 0; i < len; i++)
                {
                    ...
                    if (i != currentDropIndex)
                    {
                        ...
                        if ((i < currentDropIndex && i < dropIndex) || (i > currentDropIndex && i > dropIndex)) // Below or above the range of dragging.
                            index = i;
        ...
        public function RemoveSelf():void
        {
            ...
            {
                for each (var kid:*in container.getChildren())
                {
                    if (kid is Pod)
                    {
                        this.removeAllEventListener(kid);
            ...
            this.items = null;
            this.maximizedPod = null;
            this.minimizedItems = null;
            this.dragHighlightItems = null;
        }
    }
}